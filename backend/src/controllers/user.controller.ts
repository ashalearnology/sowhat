...
export const login = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const { accessToken, refreshToken } = await generateAccessAndRefreshToken(new Types.ObjectId(user._id));
...
export const refreshAccessToken = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const decodedToken = jwt.verify(incomingRefreshToken, process.env.JWT_SECRET!) as { id: string };
